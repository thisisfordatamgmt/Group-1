...
The task is to model and build a relational database for a telehealth and provider portal. The central entities are patients, physicians, appointments, and medical encounters. These entities work together to represent the core workflow of the healthcare delivery process. The model also includes entities like prescriptions, medications, insurance claims, insurance providers, billing, medical history, and physician specialties. The purpose of these entities is to capture clinical and administrative processes. 

  We are interested in precisely modeling each entity's relationship, generating sample data, and using this sample data to populate each entity and its attributes. We will demonstrate how the database will reflect key business rules, including physicians having multiple specialties, appointments being categorized by status, and medical encounters not being closed until billing is tied to an insurance claim. Additionally, we will perform function queries on this data, allowing us to gain valuable business insights into patient care, provider activity, appointment flow, prescription usage, and insurance reimbursement operations.

# Data Model

## Explanation:
Our model is based on the structure of a hypothetical medical clinic or healthcare practice. The central entity in the model is the Patients entity. It includes the demographic and contact information of all the patients at the clinic.

There are several important branches in the Patients entity. The Medical_History table stores all the ongoing clinical notes for the patients. A patient can have many medical history records so there is a one to many relationship between Patients and Medical_History.
Patients and appointments can be represented by a one to many relationship because one patient can book multiple appointments. The Appointments table also includes information about the appointment and it has a lookup entity labeled Appointment_Statuses which can have the statuses of "Scheduled," "Completed, and " "No-Show".

Each appointment includes a Medical_Encounters record, and each medical encounter only has one appointment, so they have a one to one relationship. The Medical_Encounters entity includes the clinical details of the visit, including encounter notes, the date, and the encounter's status. There is also an Encounter_Statuses lookup table which includes the encounter statuses.

Each Physician can have many medical encounters so there is a one to many relationship between the entities. The physicians table includes the physician's name and start date. Each physician can have multiple specialties, and a specialty can belong to many physicians, so they have a many to many relationship. This creates the Physician_Has_Specialty associative entity.

During a medical encounter, physicians prescribe medications to a patient. One patient can have multiple medications, and one medication can be prescribed to multiple patients. This creates a many to many relationship between patients and medications which results in the entity prescriptions. Each medical encounter can also have many prescriptions prescribed so there is a one to many relationship between these two entities. 

On the billing and insurance side of the model, each medical encounter can generate a billing record, represented by the one to many relationship between Medical_Encounters and Billing. The Billing table tracks the amount outstanding, amount paid, the billing date, and links to an insurance claim. Insurance_Providers contains the provider's name and any additional notes. 
